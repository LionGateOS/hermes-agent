...
def build_skills_system_prompt(
    ...
    if not skills_by_category:
        ...
        result = (
            "## Skills (scope-first)\n"
            "Before replying, identify the user's current task scope. Load a skill only when it is "
            "directly relevant to that current task, not merely because it mentions a related project, "
            "website, category, or old priority. Do not let available skills override the user's current "
            "instruction. If the task is Hermes reliability, Hermes configuration, Pixie behavior, "
            "tool safety, memory, sessions, gateway, voice, or local agent operation, keep the scope on "
            "Hermes and do not load LionGateOS Travels, SEO, website-upgrade, deployment, or marketing "
            "skills unless the user explicitly asks for that area. Prefer fewer, better-matched skills "
            "over broad loading. Skills contain useful workflows, but stale or adjacent skills can create "
            "task drift, so scope discipline comes first.\n"
